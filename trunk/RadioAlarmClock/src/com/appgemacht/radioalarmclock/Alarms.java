...
/**
 *  Verwalte Alarme, ein Alarm-Objekt pro Wochentag
 *  Da dayOfWeek von 1..7 läuft sind einige Umrechnungen notwendig
 *
 */
public class Alarms {

	public Alarm[] alarmsPerDayOfWeek=new Alarm[8]; // 0 ignoriert,1=So,2=Mo,...,7=Sa
	...
	public List<List<Alarm>> alarmGroups()
	{
		...
		weekdayLoop: for(int day:Alarm.weekdays())
		{
			...
			if(!alarm.active)
				continue;
			CharSequence alarmstr=Alarm.timeAsString(alarm.hours, alarm.minutes);
			// haben wir schon einen Eintrag mit dieser Alarmzeit?
			for(List<Alarm> group:groups)
			{
				Alarm firstAlarmInGroup=group.get(0);
				if(alarmstr.equals(Alarm.timeAsString(firstAlarmInGroup.hours, firstAlarmInGroup.minutes)))
				{
					group.add(alarm);
	...
	public Alarm nextAlarm()
	{
		...
		for(int i=0;i<alarmsPerDayOfWeek.length;i++)
		{
			...
			if(a.active)
			{
				if(nextAlarm==null || nextAlarm.calendar().after(a.calendar()))
					nextAlarm=a;
			}
		}
		return nextAlarm;
	...
	private void loadFromJSON(String jsonstr)
	{
		try {
			...
			for(int i=0;i<jsonAlarms.length() && i<alarmsPerDayOfWeek.length;i++)
			{
				alarmsPerDayOfWeek[i]=new Alarm(jsonAlarms.getJSONObject(i),i);
			}
	...
	public void scheduleNextAlarm(Context context) {
		...
		Log.v("Alarm","Schedule next alarm for "+
			SimpleDateFormat.getDateTimeInstance().format(alarm.calendar().getTime()));
